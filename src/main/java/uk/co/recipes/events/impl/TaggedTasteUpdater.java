...
public class TaggedTasteUpdater implements IEventListener {

    private static final Logger LOG = LoggerFactory.getLogger(TaggedTasteUpdater.class);
    ...
    private static final float OPTIONAL_INGREDIENT_WEIGHT = 0.4f;
    private static final float MATCHING_RECIPE_CATEGORY_WEIGHT = 0.6f;
    private static final float INGREDIENT_CONSTITUENT_WEIGHT = 0.25f;  // Pretty low, but bear in mind that if a constit is tagged with 'MEAT', that could still pull in +10

    @Inject
    public TaggedTasteUpdater() {
        // For Dagger
    }
    ...
    private void addRecipeIngredients(final long inRecipeId, final Locale inRecipeLocale, final Map<ITag,Serializable> inRecipeTags, final Collection<IIngredient> inIngredients) {
        ...
        try {
            ...
            if (changesMade) {
                myrrix.ingest(new StringReader(myrrixPrefsBuf.toString()));
    ...
    private boolean doSetTag(final String inString, final String inSetStr, final long inItemOrRecipeId, final float inScoreToUse) throws TasteException {
        ...
        return myrrix.setItemTag(inString, inItemOrRecipeId, inScoreToUse);
    }
    ...
    private void rateGenericItem(final long inUserId, final long inGenericItemId, final float inRating) {
        try {
            myrrix.ingest(new StringReader(inUserId + "," + inGenericItemId + "," + inRating));
        } catch (TasteException e) {
            throw new RuntimeException(e);
        }
    }
